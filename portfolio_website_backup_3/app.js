...
/* ========================================
   THREE.JS — GODTIER AURORA NEBULA
   ======================================== */
try {
(function() {
    ...
    const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            // Fractal Brownian Motion - 5 octaves for silky detail
            float fbm(vec2 p) {
                float val = 0.0;
                float amp = 0.5;
                float freq = 1.0;
            ...
            // Domain warping for organic, flowing distortion
            ...
            void main() {
                ...
                // Mouse interaction - smooth warp
                vec2 mouse = u_mouse;
                mouse.x *= aspect;
                float mDist = length(st - mouse);
                st += (st - mouse) * smoothstep(0.6, 0.0, mDist) * 0.08;

                // Domain-warped fractal noise
                float n = warpedFbm(st * 0.8, u_time);

                // Five color palette - premium gradients
                vec3 deepBlue   = vec3(0.02, 0.15, 0.4);
                vec3 electricCyan = vec3(0.0, 0.85, 1.0);
                vec3 royalPurple  = vec3(0.45, 0.1, 0.85);
                vec3 hotPink      = vec3(0.95, 0.15, 0.45);
                vec3 warmOrange   = vec3(1.0, 0.5, 0.15);

                // Layered color mixing
                vec3 col = deepBlue;
                col = mix(col, electricCyan, smoothstep(-0.3, 0.3, n));
                col = mix(col, royalPurple, smoothstep(-0.1, 0.6, sin(n * 3.0 + u_time * 0.3)));
                col = mix(col, hotPink, smoothstep(0.2, 0.8, sin(n * 2.0 - u_time * 0.2 + 1.5)));
                col = mix(col, warmOrange, smoothstep(0.5, 1.0, n) * 0.3);

                // Brighten peaks
                col += vec3(0.15) * smoothstep(0.3, 0.8, n);

                // Organic vignette
                float vig = smoothstep(1.6, 0.3, length(vUv - 0.5));

                // Alpha: visible but not overwhelming
                float alpha = smoothstep(-0.4, 0.6, n) * 0.35 * vig;

                // Mouse glow hotspot
                float mouseGlow = smoothstep(0.4, 0.0, mDist) * 0.15;
                alpha += mouseGlow;
    ...
    function animate() {
        ...
        uniforms.u_mouse.value.x += (targetMouseX - uniforms.u_mouse.value.x) * 0.03;
        uniforms.u_mouse.value.y += (targetMouseY - uniforms.u_mouse.value.y) * 0.03;
