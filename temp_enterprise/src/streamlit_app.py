...
with st.sidebar:
    ...
    if env_api_key:
        st.success("✅ API Key configured via server secrets")
    else:
        # Only show input if no env secret is set
    ...
    st.markdown(
        "- *Analyze the Merger Agreement from Legal with 105 pages. Escalate if high risk.*\n"
        "- *Review the Financial Audit 2025 from Finance. Notify cfo@company.com if high risk.*\n"
        "- *Check the compliance policy for the Legal department.*\n"
        "- *Search the knowledge base for tax documents.*"
    )
