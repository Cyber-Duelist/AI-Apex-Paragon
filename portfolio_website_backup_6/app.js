/* ========================================
   ULTRA-ALIEN BOOT SEQUENCE & MATRIX DATA-RAIN
   ======================================== */
(function() {
    // ---- 1. BETA CAPSULE FLASH INTRO ----
    ...
    if (canvas) {
        ...
        const alienChars = "アァカサタナハマヤャラワガザダバパイィキシチニヒミリヰギジヂビピウゥクスツヌフムユュルグズブヅプエェケセテネヘメレゲゼデベペオォコソトノホモヨョロゴゾドボポヴッン△▽○◇".split("");
        const fontSize = 16;
        ...
        let lastTime = 0;
        const fps = 25; // Slower matrix effect
        ...
        function drawMatrix(time) {
            ...
            for (let i = 0; i < drops.length; i++) {
                const text = alienChars[Math.floor(Math.random() * alienChars.length)];
...
/* ========================================
   THREE.JS — HYPER-SPEED SPECIUM RAYS
   ======================================== */
try {
(function() {
    ...
    for(let i = 0; i < starCount; i++) {
        ...
        velocities[i] = 2 + Math.random() * 8; // speed
    }

    geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    
    // Cyan and Red light streaks
    ...
    function animate() {
        ...
        for(let i=0; i < starCount; i++) {
            ...
            if(posAttr[i*3+2] > 600) {
                posAttr[i*3+2] = -1000;
                posAttr[i*3] = (Math.random() - 0.5) * 2000;
                posAttr[i*3+1] = (Math.random() - 0.5) * 2000;
            }
        }
        geometry.attributes.position.needsUpdate = true;

        // Change color dynamically based on time to simulate red/cyan flashing
        const t = Date.now() * 0.002;
        material.color.setHSL(0.5 + Math.sin(t)*0.1, 1.0, 0.6); // cycles between blue/cyan
        
        // Randomly flash red
        if (Math.random() > 0.98) {
            material.color.setHex(0xff2a2a);
        }
